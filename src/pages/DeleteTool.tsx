...
import { readTool, deleteTool } from "../api/api.ts";
...
const DeleteTool = () => {
  ...
  const {
    ...
    onSuccess: () => {
      ...
      userCtx?.setSnackbarMessage("Tool deleted successfully");
      userCtx?.setSnackbarOpen(true);
    },
  });

  const handleDeleteTool = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    mutate(id || "");
  };
  ...
  return (
    <div className="m-auto max-w-md">
      ...
      <form className="p-5 sm:p-10" onSubmit={handleDeleteTool}>
